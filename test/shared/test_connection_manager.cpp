...
TEST_CASE("Client parses request, server reads request no session key") {
    ...
    ClientToServerManager client{"323994cfb9da285a5d9642e1759b224a",
                                 "server_pub.pem"};
    GenericServerManager server{"server_priv.pem",
                                "Hello, world! 2.0 password"};

    AuthenticateRequest registration("alice", {});
    Request request{{Request::Type::LOGIN, 28}, registration.serialize()};
...
TEST_CASE("Client parses request, server reads request session key") {
    ClientToServerManager client{"73bed6b8e3c1743b7116e69e22229516",
                                 "server_pub.pem"};
    client.switchSecureChannel(true);    // enable secure channel

    ServerToClientManager server{"73bed6b8e3c1743b7116e69e22229516"};

    Request request{{Request::Type::REMOVE, 1111},
                    std::vector<unsigned char>{5, 15, 15, 1, 99, 32, 13, 13, 15,
                                               6, 51, 32, 13, 2}};
...
TEST_CASE("Server parses response, client reads response session key") {
    ...
    Response response{{Response::Type::DATABASE_NOT_FOUD, 1111},
                      std::vector<unsigned char>{5, 15, 15, 1, 99, 32, 13, 13,
                                                 15, 6, 51, 32, 13, 2}};
