...
TEST_CASE("TEST VECTORS") {
    ...
    SECTION(
        "TEST 2 : Test with a key shorter than the length of the HMAC "
        "output.") {
    ...
    SECTION(
        "TEST 3 : Test with a combined length of key and data that is larger "
        "than 64 bytes") {
    ...
    SECTION(
        "TEST 4 : Test with a combined length of key and data that is larger "
        "than 64 bytes") {
        for (unsigned char i = 1; i < 26; i++) key.push_back(i);
    ...
    SECTION("TEST 7 : Test with a key and data that is larger than 128 bytes") {
        ...
        data =
            ...
            "The key needs to be hashed before being used by the HMAC "
            "algorithm.";
    }
