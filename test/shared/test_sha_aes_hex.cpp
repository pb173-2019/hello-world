...
TEST_CASE("HexUtils") {
    unsigned char data[16]{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                           0x08, 0x09, 0xa1, 0xd2, 0x55, 0x6a, 0xff, 0x51};
...
TEST_CASE("SHA-512") {
    ...
    SECTION("ALPHABET 2") {
        std::stringstream alphabet2{
            "abcdefghbcdefghicdefghijdefghijkefghijklfghij"
            "klmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrst"
            "u"};
        CHECK(hash.getHex(alphabet2) ==
...
TEST_CASE("ALL: AES-128 custom msg with PKCS7 padding") {
    ...
    SECTION("Long text") {
        ...
        msg =
            ...
            "some inwrd qualms,\n"
            "when he reached the Father Superior's with Ivan: he felt ashamed "
            "of havin lost his temper.\n"
            "He felt that he ought to have disdaimed that despicable wretch, "
            "Fyodor Pavlovitch,\n"
            "too much to have been upset by him in Father Zossima's cell, and "
            "so to have forgotten himself.\n"
            "\"Teh monks were not to blame, in any case,\" he reflceted, on "
            "the steps. \"And if they're decent\n"
            "people here (and the Father Superior, I understand, is a "
            "nobleman) why not be friendly and\n"
            "courteous withthem? I won't argue, I'll fall in with everything, "
            "I'll win them by politness,\n"
            "and show them that I've nothing to do with that Aesop, thta "
            "buffoon, that Pierrot, and have\n"
...
TEST_CASE("ALL: AES-128 custom msg with PKCS7 padding and generated KEY") {
    ...
    AES128 aes3{};    // test pseudo-randomness
    CHECK(aes1.generateKey() != aes1.getKey());
    CHECK(aes3.generateKey() != aes1.getKey());
    CHECK(aes2.generateKey() != aes1.getKey());
}
