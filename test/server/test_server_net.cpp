...
void server_recieve_1ucnm(int n, std::string msg) {
    SECTION("1 unregistered client sending " + std::to_string(n) + "x" + msg) {
        ...
        for (auto &i : call.received) CHECK(i.second == msg);
    }
}
...
void server_recieve_1rcnm(int n, std::string /*unused*/, std::string msg) {
    SECTION("1 registered client sending " + std::to_string(n) + "x" + msg) {
        ...
        QObject::connect(&server, SIGNAL(recieved(QHostAddress, quint16)), &r,
                         SLOT(onEmmit(QHostAddress, quint16)));
...
void server_send_1rcnm(int n, std::string /*unused*/, std::string msg) {
    SECTION("1 registered client recieving " + std::to_string(n) + "x" + msg) {
        ...
        // QObject::connect(&server, SIGNAL(recieved(QHostAddress, quint16 )),
        // &r, SLOT(onEmmit(QHostAddress, quint16 )));

        QObject::connect(&call, SIGNAL(done()), &a, SLOT(quit()));

        client.onMessageRecieved = [&, c = 0](std::string /*unused*/) mutable {
            ++c;
            if (c == n) a.quit();
        };
...
void server_send_1ucnm(int n, std::string msg) {
    SECTION("1 unregistered client receiving " + std::to_string(n) + "x" +
            msg) {
        int argc = 0;
        char name[] = "Test";
        char *argv[] = {name, NULL};
        ...
        QObject::connect(&server, SIGNAL(conn(QHostAddress, quint16)), &r,
                         SLOT(onEmmit(QHostAddress, quint16)));
...
void nrclients_echo(std::vector<std::string> names, std::string msg) {
    SECTION("ehoing messages of " + std::to_string(names.size()) + " users") {
        ...
        for (auto &i : names) {
            ...
            client->onConnect = [&msg](MocClient *c) { c->send(msg); };
...
TEST_CASE("connect and send message") {
    ...
    std::string msg;
    SECTION("short message") { msg = "O"; }
    SECTION("normal length message") { msg = "Hello"; }
    SECTION("super long message") { msg = std::string(1000, 'a'); }
...
TEST_CASE("connect and receive message") {
    ...
    client.onMessageRecieved = [&msg](std::string s) { CHECK(s == msg); };
...
TEST_CASE("registered send multiple messages") {
    ...
    server_send_1rcnm(7, "Jim", "Lorem ipsum dolor sit amet, consectetur cras
amet.");
}
...
TEST_CASE("unregistered recieve multiple messages") {
    ...
    server_recieve_1ucnm(7, "Lorem ipsum dolor sit amet, consectetur cras
amet.");
}
...
TEST_CASE("registered recieve multiple messages") {
    ...
    server_recieve_1rcnm(7, "Jim", "Lorem ipsum dolor sit amet, consectetur cras
amet.");
}
