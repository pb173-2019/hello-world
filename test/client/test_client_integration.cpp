...
TEST_CASE("Create key for aliceabc") {
    RSAKeyGen keygen;
    keygen.savePrivateKeyPassword("aliceabc_priv.pem", "hunter28");
...
bool checkContains(const std::map<uint32_t, std::string>& values,
...
TEST_CASE(
    "Messages exchange - two users online, establish the X3DH shared secret "
    "connection") {
    Network::setEnabled(true);
    Network::setProblematic(false);
    ...
    for (auto it : aliceabc.getUsers())
        if (it.second == "bob") id = it.first;

    REQUIRE(id == bob.getId());

    SECTION("aliceabc uses the avaliable key from bundle of one time keys") {
        // national secret message!
        aliceabc.sendData(id, std::vector<unsigned char>{'a', 'h', 'o', 'j',
                                                         'b', 'o', 'b', 'e'});

        SendData received = bob.getMessage();
        CHECK(received.from == "aliceabc");
        CHECK(received.data == std::vector<unsigned char>{'a', 'h', 'o', 'j',
                                                          'b', 'o', 'b', 'e'});
    }

    SECTION(
        "Bob updates his bundle, but is able to decrypt the message anyway") {
        bob.sendKeysBundle();
    ...
    SECTION("Some mischievous user has emptied the one time keys") {
        for (int i = 0; i < 20; ++i) {
            // no message, the client attempts to send no existing data (file)
            // with the key bundle
        ...
        uint64_t lastTimeStamp =
            server.getDatabase().getBundleTimestamp(bob.getId());
        std::vector<unsigned char> oldBundle =
            server.getDatabase().selectBundle(bob.getId());

        // bob now goes offline, and when logs in, the server requests key
        // update / the onetime keys were gone
        bob.logout();
        ...
        uint64_t newTimeStamp =
            server.getDatabase().getBundleTimestamp(bob.getId());
        std::vector<unsigned char> newBundle =
            server.getDatabase().selectBundle(bob.getId());
        ...
        CHECK(newTimeStamp ==
              timestampOfLogin);    // timestamp varies once a hour
        ...
        // CHECK(oldKeys.timestamp < newKeys.timestamp); will not work as the
        // timestamp changes a hour
    }
    server.dropDatabase();
    Network::setEnabled(false);
