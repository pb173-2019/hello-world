...
class Server
   ...
   public:
    ...
    void setTransmissionManager(
        std::unique_ptr<ServerTransmissionManager> &&ptr) {
     ...
     *        incoming request
     *
     * @param hasSessionKey false if first connection (e.g. session key not
     * established) maybe not needed (use empty name)
     * @param username username of incoming connection, empty if not opened
     * (e.g. authentication needed)
     * @param data decoded data, ready to process (if username empty, use server
     * key to encrypt)
     */
        ...
        try {
            ...
            if (!hasSessionKey ||
                ...
                if (existing == _connections.end()) {
                    ...
                    request = pending->second.first->manager->parseIncoming(
                        std::move(data));
                } else {
            ...
            sendReponse(
                username,
                {{Response::Type::GENERIC_SERVER_ERROR, request.header.userId},
                 ex.serialize()},
            ...
            sendReponse(
                username,
                {{Response::Type::GENERIC_SERVER_ERROR, request.header.userId},
                 from_string(/*p ? p.__cxa_exception_type()->name() : */
                             "unknown error")},
                getManagerPtr(username, true));
        }
    }
    ...
    // to get to database
    const ServerDatabase &getDatabase() { return *_database; }
   ...
   private:
    ...
    std::map<std::string, std::pair<std::unique_ptr<Challenge>, bool>>
        _requestsToConnect;
   ...
   public:
     ...
     *
     * @param request request containing receiver's id in header, instead of the
     ...
     * @param manager manager to use or nullptr - the server will return generic
     * error
     */
    void sendReponse(const std::string &username, const Response &response,
     ...
     * @param username username to send the response to
     * @param response response to parse
     * @param sessionKey session key to use to encrypt reponse, or empty string
     * - the server will return generic error
     */
    void sendReponse(const std::string &username, const Response &response,
                     const zero::str_t &sessionKey);
     ...
     */
    ServerToClientManager *getManagerPtr(const std::string &username,
                                         bool trusted);
