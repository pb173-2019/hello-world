...
std::queue<std::pair<std::string, std::vector<unsigned char>>> Network::delayed{};
...
void Network::release() {
    if (delayed.empty())
        return;
    std::pair<std::string, std::vector<unsigned char>> &delayedTcp = delayed.front();
...
void Network::sendToServer() {
    ...
    if (problem) {
        ...
        delayed.push(std::make_pair(tcp, data));
        message.close();
    } else {
        (server_instance->*server_callback)();
    }
