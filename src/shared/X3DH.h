...
public:
     ...
     * @return X3DH SK key
     */
    std::string out(const KeyBundle<C25519> bundle,
     ...
     * @return X3DH SK key
     */
    std::string in(SendData& toReceive,
                   const Response& incoming) {

        X3DHRequest<C25519> x3dhBundle = X3DHRequest<C25519>::deserialize(incoming.payload);
        bool old = timestamp != x3dhBundle.timestamp; //use the previous bundle version ?

        std::vector <unsigned char> dh_bytes;

        C25519 identityKeyCurve;
        identityKeyCurve.loadPrivateKey(username + idC25519priv + (old ? ".old" : ""), pwd);
        C25519 preKeyCurve;
        preKeyCurve.loadPrivateKey(username + preC25519priv + (old ? ".old" : ""), pwd);
        ...
        if (x3dhBundle.opKeyUsed == X3DHRequest<C25519>::OP_KEY_USED) {
            ...
            onetimeKeyCurve.loadPrivateKey(
                    username + std::to_string(x3dhBundle.opKeyId) + oneTimeC25519priv + (old ? ".old" : ""), pwd);
...
private:
     ...
     */
    void append(std::vector<unsigned char>& to, const std::vector<unsigned char> &from) const {
     ...
     */
    std::vector<unsigned char> loadC25519Key(const std::string& filename) const {
